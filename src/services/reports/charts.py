...
def generate_lines_chart(data: ChartLinesData) -> bytes:
    ...
    if has_legend:
        ...
        legend_ax.axhline(y=0.82, xmin=0.05, xmax=0.95,
                          color='#2E2E2E', linewidth=0.8)
    ...
    # ── Decoração ────────────────────────────────────────────────────────────
    fig.text(0.97, 0.03, '✦', ha='right', va='bottom',
             fontsize=18, color='#2A2A3A', transform=fig.transFigure)

    # ── Exportar ─────────────────────────────────────────────────────────────
    buf = io.BytesIO()
    fig.savefig(buf, format='png', bbox_inches='tight',
                dpi=130, facecolor=BG, edgecolor='none')
    buf.seek(0)
    return buf.getvalue()
