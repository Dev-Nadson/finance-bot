...
from services.repositories.expenses_repository import create_expense_repo, list_expenses_repo
...
async def start_expense(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and len(context.args) >= 4:
        ...
        if err:
            ...
            await update.message.reply_text(f"Despesa '{name}' de R$ {value:.2f} registrada com sucesso!")
        return ConversationHandler.END
