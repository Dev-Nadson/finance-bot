...
async def create_expense_repo(account_id: int, value: float, type: str, category: str, description: str, telegram_id: str):
    async with get_session() as session:
        ...
        expense = Expenses(account_id=account_id, user_id=user.user_id, value=value, type=type, category=category, description=description)
        session.add(expense)
        await session.flush()
...
async def delete_expense_repo(expense_id: int, telegram_id: str):
    async with get_session() as session:
        ...
        expense = (await session.execute(select(Expenses).filter_by(expenses_id=expense_id, user_id=user.user_id))).scalar_one_or_none()
        if not expense:
